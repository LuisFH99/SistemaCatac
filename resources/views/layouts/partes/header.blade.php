
<header id="header" class="header-one">
  <div style="background: #fff5e3">
    <div class="container">
      <div class="logo-area">
          <div class="row align-items-center">
            <div class="logo col-lg-3 text-center text-lg-left mb-5 mb-md-5 mb-lg-0">
                <a class="d-block" href="/inicio">
                  <div class="btn-group" role="group" aria-label="Basic example">
                      <img loading="lazy" src="{{ asset('/img/logo_catac.png') }}" style="width: 45%; height: 45%;"  alt="Comunidad de catac">
                      &nbsp;&nbsp;&nbsp;&nbsp;<b style="font-size: 30px; margin-top: 7%; line-height : 30px;">COMUNIDAD CAMPESINA DE CATAC</b> 
                  </div>
                </a>
              </div>
            <div class="col-lg-9 header-right">
                <ul class="top-info-box">
                  <li>
                    <div class="info-box">
                      <div class="info-box-content">
                          <p class="info-box-title">Llamenos</p>
                          <p class="info-box-subtitle"><a href="tel:(043) 444687">(043) 444687</a></p>
                      </div>
                    </div>
                  </li>
                  <li>
                    <div class="info-box">
                      <div class="info-box-content">
                          <p class="info-box-title">Nuestro Correo</p>
                          <p class="info-box-subtitle"><a href="mailto:comunidad.decatac@gmail.com">comunidad.decatac@gmail.com</a></p>
                      </div>
                    </div>
                  </li>
                  <li class="last">
                    <div class="info-box last">
                      <div class="info-box-content">
                          <p class="info-box-title">Reconocimiento</p>
                          <p class="info-box-subtitle">Decreto Supremo N°2255885255</p>
                      </div>
                    </div>
                  </li>
                  <!--
                  <li class="header-get-a-quote">
                    <a class="btn btn-primary" href="contact.html">Contacto</a>
                  </li> -->
                </ul><!-- Ul end -->
            </div>
          </div><!-- logo area end -->
  
      </div><!-- Row end -->
    </div><!-- Container end -->
  </div>

  <div class="site-navigation">
    <div class="container">
        <div class="row">
          <div class="col-lg-14">
              <nav class="navbar navbar-expand-lg p-0">
                <button class="navbar-toggler" type="button" data-toggle="collapse" data-target=".navbar-collapse" aria-controls="navbar-collapse" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
                </button>
                
                <div id="navbar-collapse" class="collapse navbar-collapse">
                    <ul class="nav navbar-nav mr-auto">
                      <li class="nav-item dropdown active">
                          <a href="/inicio" class="nav-link dropdown-toggle">Inicio </i></a>
                      </li>

                      <li class="nav-item dropdown">
                          <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">Comunidad</a>
                          <ul class="dropdown-menu">
                            <li><a href="/reseñahistorica">Reseña Historica</a></li>
                            <li><a href="/visionmision">Misión y Visión</a></li>
                            <li><a href="/objetivos">Objetivos</a></li>
                            <li class="dropdown-submenu">
                              <a href="#!" class="dropdown-toggle" data-toggle="dropdown">Organos de gobierno</a>
                              <ul class="dropdown-menu">
                                <!-- <li><a href="/comitespecializado">Comites Especializados</a></li>-->
                              <li><a href="/organosdedireccion">Organos de Direccion</a></li>
                              <li><a href="/organosdejecucion">Organos de Ejecucion</a></li>
                              <li><a href="/organosdeadministracion">Organos de Administracion</a></li>
                              <li><a href="/organosdelinea">Organos de Linea</a></li>

                              <!-- <li><a href="/organosdeapoyo">Organos de Asesoria y Apoyo</a></li> -->
                              <!-- <li class="dropdown-submenu">
                                <a class="test" tabindex="-1" href="#">Organos de Linea <span class="caret"></span></a>
                                <ul class="dropdown-menu">
                                  <li><a tabindex="-1" href="/organosdelinea">Actividades Productivas</a></li>
                                  <li><a tabindex="-1" href="/aempresariales">Actividades Empresariales</a></li>
                                </ul>
                              </li> -->

                              
                              <li class="dropdown-submenu">
                                <a class="test" tabindex="-1" href="#">Organos Autonomos <span class="caret"></span></a>
                                <ul class="dropdown-menu">
                                  <li><a tabindex="-1" href="/controlinterno">Control Interno</a></li>
                                  <li><a tabindex="-1" href="/comitelectoral">Comite Electoral</a></li>
                                </ul>
                              </li>
                              <li class="dropdown-submenu">
                                <a class="test" tabindex="-1" href="#">Organos de Asesoria y Apoyo <span class="caret"></span></a>
                                <ul class="dropdown-menu">
                                  <li><a tabindex="-1" href="/organosdeapoyo">Organos de Apoyo</a></li>
                                  <li><a tabindex="-1" href="/organosdeasesoria">Organos de Asesoria</a></li>
                                  <li><a tabindex="-1" href="/comites">Comites Especializados</a></li>
                                </ul>
                              </li>
                            </ul>
                          </li>
                          <li><a href="/directorio">Directorio</a></li>
                          </ul>
                      </li>
              
                      <li class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">Instrumentos de Gestión</a>
                        <ul class="dropdown-menu" role="menu">
                          <li><a href="/estatuto">Estatuto</a></li>
                          <li class="dropdown-submenu">
                            <a href="#!" class="dropdown-toggle" data-toggle="dropdown">Instrumentos de redaccion</a>
                            <ul class="dropdown-menu">
                              <li><a href="/rof">ROF</a></li>
                              <li><a href="/poi">POI</a></li>
                              <li><a href="/peconvenios">PEC - Convenio</a></li>
                              <li><a href="/normativas">DOCUMENTOS NORMATIVOS</a></li>
                              
                            </ul>
                          </li>
                        </ul>
                    </li>
            
                    <li class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">Servicios</a>
                        <ul class="dropdown-menu" role="menu">
                          <li class="dropdown-submenu">
                            <a href="#!" class="dropdown-toggle" data-toggle="dropdown">Servicentro</a>
                            <ul class="dropdown-menu">
                              <li><a href="/serviciosprincipal">Gasolinera</a></li>
                              <li><a href="/serviciosprincipalminimarket">Minimarket</a></li>
                              <li><a href="/serviciosprincipalrestaurante">Restaurant</a></li>
                            </ul>
                          </li>
                          <li class="dropdown-submenu">
                            <a href="#!" class="dropdown-toggle" data-toggle="dropdown">Agropecuaria</a>
                            <ul class="dropdown-menu">
                              <li><a href="/serviciosprincipalganaderia">Ganadería</a></li>
                              <li><a href="/serviciosprincipalagricultura">Agricultura</a></li>
                            </ul>
                          </li>
                          <li class="dropdown-submenu">
                            <a href="#!" class="dropdown-toggle" data-toggle="dropdown">Transporte</a>
                            <ul class="dropdown-menu">
                              <li><a href="/serviciosprincipaltransporte">Transporte de materiales</a></li>
                            </ul>
                          </li>
                          <li><a href="/serviciosprincipalcantera">Cantera</a></li>
                          <li><a href="/serviciosprincipalagroveterinaria">Agroveterinaria</a></li>
                          <li><a href="/serviciosprincipalturismo">Turismo</a></li>
                          <li><a href="/serviciosprincipalforestacion">Forestación</a></li>
                        </ul>
                    </li>
            
                    <li class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">Noticicas y Eventos</a>
                        <ul class="dropdown-menu" role="menu">
                          <li><a href="/noticias">Noticias</a></li>
                          <li><a href="/eventos">Eventos</a></li>
                          <li><a href="/actividades">Actividades Programadas</a></li>
                        </ul>
                    </li>
                    </ul>
                </div>
              </nav>
          </div>
          <!--/ Col end -->
        </div>
        <!--/ Row end -->
    </div>
    <!--/ Container end -->

  </div>
  <!--/ Navigation end -->
</header>

